#!/bin/bash
# p3cfg3.sh -- Option A refined: vision + 192,512 with slimmed media budgets (frees ~1.75 GiB
...
nohup /opt/ninfer/src/build/apps/ninfer-serve /opt/ninfer/models/qwen3_8_27b_nvfp4.ninfer \
  --api-key "$AK" --max-context 152576 --kv-dtype int8 --max-concurrency 2 \
  --spec mtp --draft-tokens 3 --lm-head-draft --vision \
  --media-cache-mib 256 --media-live-mib 1024 \
  > /opt/ninfer/logs/p3cfg3.out 2> /opt/ninfer/logs/p3cfg3.err &
UP=0
...
grep -aoE "free-after-startup=[0-9.]+ MiB|media-cache=[0-9.]+ GiB|media-live=[0-9.]+ GiB" /opt/ninfer/logs/p3cfg3.err | head -3
if [ "$UP" = "1" ]; then
  cd $R && TARGET_URL=http://127.0.0.1:8080/v1/chat/completions NEEDLE_SIZES=140000 $PY -u needleprobe.py 2>&1 | tail -2
  cd $R && QWEN_URL=http://127.0.0.1:8080 $PY -u vision-probe.py 2>&1 | tail -3
else
  echo "boot failed:"; tail -c 900 /opt/ninfer/logs/p3cfg3.err
fi
